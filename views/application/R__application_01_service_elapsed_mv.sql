...
select EXTRACT(year from bra.timestamp_tz) as year,EXTRACT(month from bra.timestamp_tz) as month, EXTRACT(day from bra.timestamp_tz) as day,  bra.service, bra.endpoint, bra.http_method, AVG(era."timestamp" - bra."timestamp") as ms_avg_elapsed  from interop_dev.application.begin_request_audit bra inner join
